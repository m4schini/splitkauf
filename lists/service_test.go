...
func TestAddItem(t *testing.T) {
	...
	it, err := svc.AddItem(ctx, l.ID, "Milk", 0, "", new("   "), false, testActor)
	if err != nil {
		t.Fatalf("AddItem: %v", err)
	}
	...
	it2, err := svc.AddItem(ctx, l.ID, "Eggs", 12, "", new("free range"), false, testActor)
	if err != nil {
		t.Fatalf("AddItem: %v", err)
	}
...
func TestUpdateItem(t *testing.T) {
	...
	ctx := context.Background()
	l, _ := svc.CreateList(ctx, "Groceries", testActor)
	it, _ := svc.AddItem(ctx, l.ID, "Milk", 1, "", new("2%"), false, testActor)

	// invalid name
	if _, err := svc.UpdateItem(ctx, l.ID, it.ID, ItemUpdate{Name: new("")}); err == nil {
		t.Fatal("expected name validation error")
	} else {
		assertValidationError(t, err, "name")
	}
	...
	// invalid quantity
	if _, err := svc.UpdateItem(ctx, l.ID, it.ID, ItemUpdate{Quantity: new(0)}); err == nil {
		t.Fatal("expected quantity validation error")
	} else {
		assertValidationError(t, err, "quantity")
	}
...
func TestUpdateItemUnit(t *testing.T) {
	...
	// valid change
	got, err := svc.UpdateItem(ctx, l.ID, it.ID, ItemUpdate{Unit: new("kg")})
	if err != nil {
		t.Fatalf("UpdateItem: %v", err)
	}
	...
	// invalid token
	if _, err := svc.UpdateItem(ctx, l.ID, it.ID, ItemUpdate{Unit: new("furlong")}); err == nil {
		t.Fatal("expected unit validation error")
	} else {
		assertValidationError(t, err, "unit")
	}
	...
	got2, err := svc.UpdateItem(ctx, l.ID, it.ID, ItemUpdate{Name: new("Whole Milk")})
	if err != nil {
		t.Fatalf("UpdateItem: %v", err)
	}

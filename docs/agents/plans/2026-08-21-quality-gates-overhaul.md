...
- Post-push fixes (2026-08-21): (1) govulncheck failed in CI — `go-version-file: go.mod` installed exactly go 1.26.0 and 24 stdlib CVEs are fixed in 1.26.1+; `check-latest: true` proved ineffective (an exact three-part spec never re-resolves), the working fix is a `toolchain go1.26.6` directive in go.mod (go 1.26.0 stays the minimum). (2) trivy's blocking HIGH/CRITICAL scan flagged x/crypto v0.46.0 (ssh CVEs) and x/mod v0.37.0; bumped to v0.55.0/v0.40.0. After both fixes CI + quality green on push; workflow_dispatch (weekly vuln job) green — trivy did not fire on docs/agents/, no skip-dirs entry needed; test-full job ran adapters/db in 2.2s with zero DSN skips.
